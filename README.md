...
This library allows you to parse wikitext to extract values or
manipulate the contents. It turns raw MediaWiki-style wikitext into a
...
- Parses common wikitext constructs, like headings, links, and
  templates.
...
such as TemplateNodes, can have multiple groups of children: one for its
name and other for its parameters.
...
At the current version, this library does not support some complex
wikitext elements such as wikitables and HTML tags that are usually
supported in wikitext.

This project does not try to be a resilient parser and will throw an
error when it finds wikitext it can’t understand.
...
- Format pages.
- Extract information from pages, e.g. collecting values in specific
