...
describe('template node', () => {
  ...
  it('get mixed unnamed parameter', ({ task }) => {
  ...
  it('set mixed unnamed parameter', ({ task }) => {
    const input = '{{template|1|2=value|test}}'
    const page = parse(input);
    const template = page.children.at(0) as WikiTemplateNode;
  ...
  it('set new mixed unnamed parameter', ({ task }) => {
    const input = '{{template|1|2=value|test}}'
    const page = parse(input);
    const template = page.children.at(0) as WikiTemplateNode;
  ...
  it('set explicit unnamed parameter', ({ task }) => {
    const input = '{{template|1|2=value}}'
    const page = parse(input);
    const template = page.children.at(0) as WikiTemplateNode;
    template.set(2, 'test');
